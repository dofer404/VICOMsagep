...
class ci_tiposdepersonas extends sagep_ci
{
		...
		try {
			...
			if (adebug::$debug) {
				...
				if ($sql_state == 'db_23505') {
					throw new toba_error_usuario('Ya existe el Tipo de Persona');
				}
			}
		}
	}
	...
	function conf__cuadro(sagep_ei_cuadro $cuadro)
	{
		if (isset($this->s__datos_filtro)) {
			...
			$datos = dao_tiposdepersonas::get_listado_tipos_personas($sql_where);
	...
	function evt__form__modificacion($datos)
	{
		...
		$this->cn()->set_tipos_personas($datos);
	}
	...
	function conf__form(sagep_ei_formulario $form)
	{
		if (isset($this->s__datos['form'])) {
			...
			if ($this->cn()->hay_cursor()) {
				$datos = $this->cn()->get_tipos_personas();
				$this->s__datos['form'] = $datos;
